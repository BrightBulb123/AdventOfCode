...
#define IDX_INV -1   // Index invalid value
...
int bin_search_idx(int *list, int elem, int size);                              // Using binary search, finds *an* occurence of `elem` in list (not necessarily the first!!!!), and returns the match's index; if not found, then IDX_INV.
int freq_in_sorted_list(int *list, int elem, int size);                         // Finds the frequency of `elem` in the given list `list` of size `size`.
...
int total_calc(int *left_list, int *right_list, int num_elems)
{
    ...
    {
        total += left_list[i] * freq_in_sorted_list(right_list, left_list[i], num_elems);
    }
...
int bin_search_idx(int *list, int elem, int size)
{
...
int freq_in_sorted_list(int *list, int elem, int size)
{
    int freq = 0;

    // Since the list is sorted, use binary search to find the first occurence
    int elem_idx = bin_search_idx(list, elem, size);
    if (elem_idx == IDX_INV) // Element not found in list
        return elem * freq;  // Returns 0

    while (list[elem_idx - 1] == elem) // Traces back till it encounters the first occurence in list.
        elem_idx--;

    while (list[elem_idx + freq] == elem)
        freq++;
