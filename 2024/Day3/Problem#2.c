...
#define TEST_FILE_PATH_2 "Day3TestInput2.txt"

#define OP_NAME "mul"     // Operation name
#define ENABLE "do()"     // Operation that enables multiplications
#define DISABLE "don't()" // Operation that disables multiplications
...
bool strvaliditychecker(char str[], size_t min_size, size_t max_size, size_t size, bool *enabled, int nums[]); // Returns if the given `str` is valid or not (including checking for enabled/disabled multiplications), as well as filling up the `nums` array with the actual numbers from the `str` in the cade of a valid `str`.
...
int main(int argc, char *argv[])
{
    ...
    int total_prod = 0;
    int nums[NUM_COUNT];
    bool enabled = true; // operations enabled by default at the start

    while (strshifter(file, str, max_size + 1) != EOF)
    {
        bool valid = strvaliditychecker(str, min_size, max_size, max_size, &enabled, nums);
        if (valid)
        {
            int sum = 1;
...
bool strvaliditychecker(char str[], size_t min_size, size_t max_size, size_t size, bool *enabled, int nums[])
{
    bool valid = true;
    int pos = 0;
    ...
    if (strncmp(str, ENABLE, strlen(ENABLE)) == 0)
        *enabled = true;
    if (strncmp(str, DISABLE, strlen(DISABLE)) == 0)
        *enabled = false;
