...
void int_arr_copy_remove(int source[], int dest[], int rem_idx, int len); // Copies the int array `source`'s contents into the int array `dest`, except for copying over the element at index `i`.
bool is_rep_safe(int report[], int rep_len, int defect_count);            // Checks if the current `report` passed in is "safe", and returns `true` if it is.
...
int safe_reps_counter(FILE *file)
{
    ...
    do
    {
        ...
        bool safe = is_rep_safe(report, rep_len, 0); // 0 defects in the beginning
...
void int_arr_copy_remove(int source[], int dest[], int rem_idx, int len)
{
    for (int i = 0, j = 0; i < len && j < len - 1; i++)
    {
        dest[j++] = source[i];
        if (i == rem_idx)
            j--;
...
bool is_rep_safe(int report[], int rep_len, int defect_count)
{
    bool safe = true;
    bool increasing = true, decreasing = true;

    for (int i = 1; i < rep_len && safe; i++)
    {
        int diff = report[i] - report[i - 1];

        if (diff == 0)
            safe = false; // Unsafe, no change detected
        else if (diff > 0)
        {
            if (!increasing)
                safe = false; // Change in gradient
        ...
        if (abs(diff) < LEAST_DIFF || abs(diff) > MOST_DIFF)
            safe = false; // Too big or little of a change
    }

    for (int i = 0; i < rep_len && !safe && defect_count < 1; i++) // Try removing an item from the report
    {
        int rep_copy[rep_len - 1];
        int_arr_copy_remove(report, rep_copy, i, rep_len);
        safe = is_rep_safe(rep_copy, rep_len - 1, defect_count + 1);
    }
