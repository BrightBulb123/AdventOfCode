...
int word_checker(char **rows, size_t word_len, size_t row_len, size_t row_count)
{
    ...
    for (int r = 0; r < row_count; r++)
    {
        ...
        for (int c = 0; c < row_len; c++)
        {
            ...
            {
                if (c > row_len - word_len) // No point, as not enough space to fit in the endpoints of the "X" shape with the `TARGET` word, so won't find a match
                    continue;

                if ((row[c] != TARGET[0] && row[c] != TARGET[word_len - 1]) ||
                    (row[c + word_len - 1] != TARGET[word_len - 1] && row[c + word_len - 1] != TARGET[0])) // -1 because `word_len` isn't zero-indexed
                    continue;                                                                              // Missing endpoints of the "X" shape with the `TARGET` word, so won't find a match

                frequency += (diagonal_right_word_checker(word, rows, r, c, word_len) &&              // diagonally - top to bottom right
                              diagonal_left_word_checker(word, rows, r, c + word_len - 1, word_len)); // diagonally - top to bottom left
            }
        }
    }
